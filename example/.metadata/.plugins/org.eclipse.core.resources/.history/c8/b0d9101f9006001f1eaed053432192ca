...
void norflash_init(void)
{
    ...
    spi2_set_speed(SPI_BAUDRATEPRESCALER_2);        /* Switch SPI2 to high-speed mode 18MHz */
    ...
    if (g_norflash_type == W25Q256)     /* If SPI FLASH is W25Q256, 4-byte address mode must be enabled */
    {
        ...
        if ((temp & 0X01) == 0)         /* If not in 4-byte address mode, switch to 4-byte address mode */
        {
            ...
            NORFLASH_CS(0);
            spi2_read_write_byte(FLASH_Enable4ByteAddr);    /* Enable 4-byte address instruction */
            NORFLASH_CS(1);
        }
    }
}
...
{
    NORFLASH_CS(0);
    spi2_read_write_byte(FLASH_WriteEnable);   /* Send write enable command */
    NORFLASH_CS(1);
}
...
static void norflash_send_address(uint32_t address)
{
    ...
    {
        spi2_read_write_byte((uint8_t)((address) >> 24)); /* Send bits 31 to 24 of the address */
    } 
    spi2_read_write_byte((uint8_t)((address) >> 16));     /* Send bits 23 to 16 of the address */
    spi2_read_write_byte((uint8_t)((address) >> 8));      /* Send bits 15 to 8 of the address */
    spi2_read_write_byte((uint8_t)address);               /* Send bits 7 to 0 of the address */
}
...
uint8_t norflash_read_sr(uint8_t regno)
{
    ...
    spi2_read_write_byte(command);              /* Send command to read status register */
    byte = spi2_read_write_byte(0xFF);          /* Read one byte */
...
void norflash_write_sr(uint8_t regno, uint8_t sr)
{
    ...
    NORFLASH_CS(0);
    spi2_read_write_byte(command);  /* Send command to read status register */
    spi2_read_write_byte(sr);       /* Write one byte */
    NORFLASH_CS(1);
}
...
uint16_t norflash_read_id(void)
{
    ...
    spi2_read_write_byte(0);                        /* Write one byte */
    spi2_read_write_byte(0);
    spi2_read_write_byte(0);
    deviceid = spi2_read_write_byte(0xFF) << 8;     /* Read high 8-bit byte */
    deviceid |= spi2_read_write_byte(0xFF);         /* Read low 8-bit byte */
    NORFLASH_CS(1);
...
void norflash_read(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    for(i = 0; i < datalen; i++)
    {
        pbuf[i] = spi2_read_write_byte(0xFF);   /* Read in a loop */
    }
...
static void norflash_write_page(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    NORFLASH_CS(0);
    spi2_read_write_byte(FLASH_PageProgram);    /* Send write page command */
    norflash_send_address(addr);                /* Send address */

    for(i = 0; i < datalen; i++)
    {
        spi2_read_write_byte(pbuf[i]);          /* Write in a loop */
    }
...
void norflash_erase_chip(void)
{
    ...
    NORFLASH_CS(0);
    spi2_read_write_byte(FLASH_ChipErase);  /* Send chip erase command */
    NORFLASH_CS(1);
...
void norflash_erase_sector(uint32_t saddr)
{
    ...
    NORFLASH_CS(0);
    spi2_read_write_byte(FLASH_SectorErase);    /* Send sector erase command */
