...
<img src="../../1_docs/3_figures/17_3_dac_sine_wave/04_tim.png">
 ...
 void dac_creat_sin_buf(uint16_t maxval, uint16_t samples)
{
  ...
  w = (2 * 3.1415926) / samples;                /* ¦Ø=2¦Ð/T */

  for (i = 0; i < samples; i++)
  {
    outdata = maxval * sin(w * i + 0) + maxval; /* y=Asin(¦Øx+¦Õ)+b */
 ...
 * @param   cndtr: Amount of data transferred in a single DMA channel
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (key == WKUP_PRES)
    {
      ...
      dac_dma_wave_enable(100, 10 - 1, 72 - 1);   /* Enable DAC DMA waveform output */
    }
    else if (key == KEY0_PRES)
    {
      dac_creat_sin_buf(2048 - 1, 10);            /* Generate sine wave data */
      dac_dma_wave_enable(10, 10 - 1, 72 - 1);    /* Enable DAC DMA waveform output */
    }
